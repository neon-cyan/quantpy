...
QINP = sys.argv[1]
STEPLIMS = int(sys.argv[2])
OUTPUT = 'x11' # Either plot to a file or x11
ANNA_DIR = 'analysis'
NM_PLOTS = [8,11,12,19]
...
with open(qoutf, 'r') as f:
    data = f.read()
q_out_data=QuanticsParsers.parse_output(data)[:STEPLIMS]
...
for i in range(sdsum.shape[0]):
    ...
    axes[1,2].plot(times, dp, label='Atom {}'.format(data_gwpx['mullikenmap'][i]))
